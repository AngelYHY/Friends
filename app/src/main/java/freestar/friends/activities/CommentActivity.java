 ...
 * 詳情界面+listview（評論）
 */
public class CommentActivity extends BaseActivity implements XListView.IXListViewListener, BGANinePhotoLayout.Delegate {
    Message message;
    ...
    EditText conments;
    freestar.friends.util.view.XListView conment_listview;
    //ListView conment_listview;//評論內容
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Log.e("FreeStar", "CommentActivity→→→onCreate:" + message);
        user = new User();
    ...
    private void initView() {
        //加头布局
        view = LayoutInflater.from(getApplicationContext()).inflate(R.layout.detail_layout, null);
        //头像
        simpleDraweeView = (SimpleDraweeView) view.findViewById(R.id.dongtai_touxiang);
        //昵称
        name = (TextView) view.findViewById(R.id.dongtai_name);
        //时间
        time = (TextView) view.findViewById(R.id.dongtai_time);
        //内容
        comtent = (TextView) view.findViewById(R.id.daongtai_content);
        //点赞图标
        zanbiao = (ImageButton) view.findViewById(R.id.dongtai_zan);
        //评论图标
        pinglunbiao = (ImageView) view.findViewById(R.id.dongtai_pinglun);
//        //九宫格
        ninegridview = (BGANinePhotoLayout) view.findViewById(R.id.ninegridview);
        //返回
        ...
        conment_listview = (freestar.friends.util.view.XListView) findViewById(R.id.commemt_listview);
        conment_listview.addHeaderView(view);
                ...
                if (e == null) {
                    if (list.size() > 0) {
                        for (int i = 0; i < list.size(); i++) {
                            ...
                            Log.e("FreeStar", "ArticleItemActivity1→→→done:" + likeM.getObjectId());
...
//                        conment_listview.stopRefresh();
                    ...
                    Log.d("FreeStar", "CommentActivity→→→done" + e.getMessage());
                }
            }
        });
        i++;
    ...
    private void initAdapter() {
        commentAdapter = new CommonAdapter<DisSay>(this, R.layout.dis_item_layout, mList) {
            ...
            protected void convert(ViewHolder holder, final DisSay s, int position) {
                ...
                Log.e("FreeStar", "CommentActivity→→→convert:" + cuser.toString());
                //赋值
                ...
                holder.setOnClickListener(R.id.sdv_head, new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        Intent intent = new Intent(CommentActivity.this, UserDataActivity.class);
    ...
    private void initClick() {
        ...
        pinglunbiao.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Log.e("FreeStar", "CommentActivity→→→onClick:pinglunbiao");
                contain_layout.setVisibility(View.VISIBLE);
                //弹开虚拟键盘和输入框
                InputMethodManagerup();
            }
        ...
        sumbit_conments.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (context.equals("")) {
                    Toast.makeText(CommentActivity.this, "写写东西吧", Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    conments.setText("");
                    Downkeyboard();
...
//                    User cuser, DisSay comment_father_user, String comment, Message message,User father_user
                    ...
                    Log.e("FreeStar", "CommentActivity→→→onClick:人对动态的评论");
                    ...
                    message.update(new UpdateListener() {
                        ...
                        public void done(BmobException e) {
                            if (e == null) {
                                Log.e("FreeStar", "CommentActivity→→→done:评论数成功+1");
                            } else {
                                Log.e("FreeStar", "CommentActivity→→→done:评论数更新失败");
                            }
                        }
                    });
                    dsf.save(new SaveListener<String>() {
                        @Override
                        public void done(String s, BmobException e) {
                            Log.e("FreeStar", "CommentActivity→→→done:+保存成功");
                            onRefresh();
                        }
                    });
    ...
    private void startQuery() {
        ...
        query.findObjects(new FindListener<DisSay>() {
            ...
            public void done(List<DisSay> list, BmobException e) {
                if (e == null) {
                    if (list.size() == 0) {
                        Toast.makeText(CommentActivity.this, "没有数据了", Toast.LENGTH_SHORT).show();
                    } else {
                        mList.addAll(list);
                        commentAdapter.notifyDataSetChanged();
                    }
